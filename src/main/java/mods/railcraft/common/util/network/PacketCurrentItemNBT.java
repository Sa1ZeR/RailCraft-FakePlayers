    ...
    public void readData(DataInputStream data) throws IOException {
        try {
            ...
            if (stack.hasTagCompound())
            {
                ...
                tagCompound.setString("title", oldNbt.getString("title"));
                tagCompound.setString("owner", oldNbt.getString("owner"));
                tagCompound.setString("dest", oldNbt.getString("dest"));
                tagCompound.setString("ownerId", oldNbt.getString("ownerId"));

                oldNbt.removeTag("title");
                oldNbt.removeTag("owner");
                oldNbt.removeTag("dest");
                oldNbt.removeTag("ownerId");

                if (!oldNbt.hasNoTags()) {
                    Game.log(Level.WARN, String.format("Player %s used packet hack", player.getDisplayName()));
                }
            }
